...
**Reverse proxy**: a shared `nginx-reverse-proxy` container (ports 80/443) plus `nginx-reverse-proxy-gen` (jwilder/docker-gen, **pinned by the sha256 digest of the multi-arch manifest list** — never a single-platform child digest: `docker pull` would succeed but `docker run` would fail under `DOCKER_DEFAULT_PLATFORM=linux/amd64`; get the right digest with `docker buildx imagetools inspect <image>:<tag>`; see commits b4ec260 and 493a7b6) regenerate nginx config from running containers' `VIRTUAL_HOST` env var. Both containers are destroyed and recreated on every `up`. The nginx template is heredoc-inlined in `Up::StartReverseProxy` (plugins/up.sh).
